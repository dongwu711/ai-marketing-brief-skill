...
使用 `scripts/query.py` 读取 `yingxiaoai.com.cn` 已发布的公开数据。只读，不需要 API Key；每次查询都会重新读取公开接口，因此无需为新一期重新安装 Skill。

## 查询方式

- 最新期：`python3 scripts/query.py latest`
- 指定日期：`python3 scripts/query.py issue --date YYYY-MM-DD`
- 搜索档案库：`python3 scripts/query.py search --query "关键词"`（覆盖全部已发布的情报、案例与工具）
